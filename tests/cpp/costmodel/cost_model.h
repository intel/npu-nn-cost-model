...
protected:
    ...
    void DoRegularTest(const GTestInput& t_in, const GTestExpectations& t_exp, const std::string& test_case = "") {
        ...
        cost_models.getModel(w1.device).get_serializer().serialize(SerializableField<std::string>("info", info_layer));

        w1.set_layer_info(info_layer);
        unsigned cost_cyc{};
        std::string info;
        ASSERT_NO_THROW(cost_cyc = cost_models.getModel(w1.device).DPU(w1, info)) << t_header << w1;
