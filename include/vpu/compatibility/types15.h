...
#ifndef VPUNN_TYPES_15_H
#define VPUNN_TYPES_15_H
...
class Inserter_Interface15 : Inserter<T> {
public:
    using Inserter<T>::insert;  ///< exposes the non virtual insert methods
    Inserter_Interface15(std::vector<T>& output): Inserter<T>(output) {
    }
    ...
    size_t insert(const VPUTensor& data, size_t offset) {
        ...
        offset = this->insert<only_simulate>(intf_15::convert<intf_15::DataType>(data.get_dtype()), offset);
        return offset;
    }
};
...
class Preprocessing_Interface15_Archetype :
        public PreprocessingInserter<T, Preprocessing_Interface15_Archetype<T, DeviceAdapter, V>> {
private:
    inline static const DPU_OperationValidator workload_validator{};  ///< sanitizer mechanisms
protected:
    friend class PreprocessingInserter<T, Preprocessing_Interface15_Archetype<T, DeviceAdapter, V>>;
    ...
    void transformOnly(const DPUWorkload& workload, size_t& debug_offset,
        ...
        Inserter_Interface15<T, DeviceAdapter> ins(destination_descriptor);
        ...
        offset = ins.template insert<only_simulate>(intf_15::convert<intf_15::ExecutionMode>(workload.execution_order),
                                                    offset);
...
public:
    ...
    Preprocessing_Interface15_Archetype()
            : PreprocessingInserter<T, Preprocessing_Interface15_Archetype<T, DeviceAdapter, V>>(size_of_descriptor) {};

    /// @brief default virtual destructor
    virtual ~Preprocessing_Interface15_Archetype() = default;
};

//---------------------------------------------------------
template <class T>
using Preprocessing_Interface15 =
        Preprocessing_Interface15_Archetype<T, NN5XInputAdapter, NNVersions::VERSION_15_NPU_RESERVED_11>;
