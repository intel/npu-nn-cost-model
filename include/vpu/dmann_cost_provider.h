...
public:
    DMANNCostProvider(const std::string& filename = "",
            ...
            : vpunn_runtime(filename, profile),
              ...
              cache(cache_size /*, preprocessing.output_size()*/, dma_cache_filename,
    ...
    DMANNCostProvider(const char* model_data, size_t model_data_length, const unsigned int batch_size,
            ...
            : vpunn_runtime(model_data, model_data_length, copy_model_data, profile),
              ...
              cache(cache_size, /* preprocessing.output_size(),*/ dma_cache_data, dma_cache_data_length),
              cache_miss_serializer(get_env_vars({"ENABLE_VPUNN_CACHE_MISS_DATA_SERIALIZATION"})
                                            .at("ENABLE_VPUNN_CACHE_MISS_DATA_SERIALIZATION") == "TRUE"),
              batch_size(batch_size) {
        if (!is_initialized()) {
...
protected:
    float infer_raw_input(const WlT& workload) const {
        ...
        if (!cached_value) {
            ...
            DMACostSerializationWrap<WlT> serialization_handler(cache_miss_serializer);
    ...
    CyclesInterfaceType infer(const WlT& workload) const {
        ...
        return post_processing.process(workload, raw_value);
    }

public:
    /* coverity[pass_by_value] */
    std::tuple<float, std::string> computeBandwidthMsg(WlT wl) const {
        ...
        {
            if (is_initialized()) {
                nn_size_div_cycle = infer_raw_input(wl);
            } else {
...
public:
    ...
    std::vector<CyclesInterfaceType> get_cost(const std::vector<WlT>& workloads) const {
        if (!is_initialized()) {
    ...
    CyclesInterfaceType get_cached(const WlT& workload, std::string* source = nullptr) const {
        if (!is_initialized()) {
    ...
    void add_to_cache(const WlT& workload, const float value) const {
        if (!is_initialized()) {
            return;
        }
    ...
    static const std::vector<std::string> get_names_for_serializer() {
        ...
        for (const auto& name : WlT::_get_member_names()) {
