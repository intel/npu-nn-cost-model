 ...
 * @details Holds reference_wrappers to avoid copying workload objects. To support a new workload type, add it to
 * VariantType. This header is intended to be included only from .cpp files to avoid pulling heavy type dependencies
 * into the lightweight http_cost_provider_intf.h interface header.
 */
struct HttpWorkloadVariant {
    using VariantType = std::variant<std::reference_wrapper<const DPUOperation>,            //
                                     ...
                                     std::reference_wrapper<const SHAVEWorkload>>;
