...
private:
    ...
    static std::vector<VPUEM_CostFunction> setCostFunction(const std::vector<CostFunction3SlopesDescriptor>& costFunction3SlopesData) {
...
public:
    PiecewiseModel(DataType dtype, unsigned int DpuFreq,
                   unsigned int ShvFreq, const std::vector<CostFunction3SlopesDescriptor>& costFunction3SlopesData,
        ...
        try {
             ...
             for (int idx = 0; idx < numBlocks.back(); idx++) {
                 if (idx == (numBlocks.back() - 1)) {
                     ...
                     blk_cycles += calc_blk_cycles(odim, vpuemObject) + int(vpuemObject.costFunction3SlopesData_.offset_) - 1;
                 } else {
