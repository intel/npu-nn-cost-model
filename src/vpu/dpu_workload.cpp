...
uint32_t DPUWorkload::hash() const noexcept {
    ...
    h = PrimitiveHash::hash_enum(h, op);
    ...
    for (const auto& k : kernels) {
        h = PrimitiveHash::hash_uint32(h, k);
    }
    for (const auto& s : strides) {
        h = PrimitiveHash::hash_uint32(h, s);
    }
    for (const auto& p : padding) {
        h = PrimitiveHash::hash_uint32(h, p);
    }
    ...
    h = PrimitiveHash::hash_float(h, act_sparsity);
    h = PrimitiveHash::hash_float(h, weight_sparsity);

    // Hash swizzling arrays
    for (const auto& swizz : input_swizzling) {
        h = PrimitiveHash::hash_enum(h, swizz);
    }
    ...
    h = PrimitiveHash::hash_uint32(h, output_write_tiles);
    ...
    h = PrimitiveHash::hash_bool(h, weight_sparsity_enabled);
    ...
    h = PrimitiveHash::hash_enum(h, get_weight_type());

    // layer_info is NOT included in hash (for cache purposes)

    // Hash optional fields
    h = PrimitiveHash::hash_bool(h, is_weightless_operation());
    h = PrimitiveHash::hash_bool(h, is_inplace_output_memory());
    h = PrimitiveHash::hash_bool(h, is_superdense());
    h = PrimitiveHash::hash_bool(h, is_input_autopad());
    h = PrimitiveHash::hash_bool(h, is_output_autopad());

    // Hash MPE engine
    h = PrimitiveHash::hash_enum(h, mpe_engine);

    // Hash reduce_minmax_op flag
    // makes old cache invalid. remove temporary if you need old cache and do not
    // use the feature
